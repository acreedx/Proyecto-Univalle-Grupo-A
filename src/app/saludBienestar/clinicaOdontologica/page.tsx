...
import { IContact, IUbicacion, IRequisitos, IHorarios, IServicio } from "@/Services";
...
const Ubication = () => {
  ...
  useEffect(() => {
    const fetchUbicationData = async () => {
      try {
        const res = await fetch('https://apisistemaunivalle.somee.com/api/Ubicaciones/getUbicacionesbyModuloId/16');
        if (!res.ok) {
...
const Contacts = () => {
  ...
  useEffect(() => {
    fetch('https://apisistemaunivalle.somee.com/api/Referencia/getReferenciasbyModuloId/16')
      .then(response => response.json())
      .then(data => {
        if (Array.isArray(data.data)) {
